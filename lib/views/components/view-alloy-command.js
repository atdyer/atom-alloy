...
export default function view_alloy_command () {
    ...
    function _on_click_show_solution () {
        event.stopPropagation();
        if (_command) {
            _command.show_solution();
        }
    }
    ...
    function _set_solution (solution) {
        if (_solution === solution) return;
        _solution = solution;
        if (_solution) {
            let sat = _solution.satisfiableSync();
            if (_is_check) {
                _set_state(sat ? 'solved-invalid' : 'solved-valid');
            } else {
    ...
    function _set_state (state) {
        ...
        if (_status_icon) {
            ...
            let can_show = (_state === 'solved-consistent') || (_state === 'solved-invalid');
            _status_icon.on('click', can_show ? _on_click_show_solution : null);
