...
import settings_view from './settings-view';
...
export default class MainView {
    ...
    show_keymaps = atom.config.get('alloy.show_keymaps');
    ...
    constructor(selection) {
        ...
        // Add header
        this.header = header_view(this.div)
            .title('Alloy Commands')
            .input('input-toggle')
            .input_callback(() => {
                atom.config.set('alloy.show_keymaps', !this.show_keymaps);
                // this.show_keymaps = atom.config.get('alloy.show_keymaps');
            });

        this.header.input().property('checked', this.show_keymaps);

        // Add settings
        this.settings = settings_view(this.div);
        ...
        this.subscriptions.add(
            atom.config.observe('alloy.show_keymaps', (is_displayed) => {
                this._showKeymaps(is_displayed);
            })
        );
    ...
    _showKeymaps(is_displayed) {
        this.show_keymaps = is_displayed;
        this.commands.selectAll('.key-bindings')
            .style('display', is_displayed ? null : 'none');
        this.commands.selectAll('.cmd-status')
            .style('display', !is_displayed ? null : 'none')
    }
