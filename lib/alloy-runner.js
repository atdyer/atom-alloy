...
export default class AlloyRunner {
    ...
    // Event subscriptions
    onJavaLoading(callback) {
        this._emitter.on('javaloading', callback);
    }

    onJavaReady(callback) {
        this._emitter.on('javaready', callback);
    }

    onNeedsRestart(callback) {
        this._emitter.on('needsrestart', callback);
    }

    onCommands(callback) {
        this._emitter.on('commands', callback);
    }

    parse(file) {
        if (this._is_java_running) {
            this.Parser.parseOneModule(file, (error, result) => { this._onParse(error, result) });
        }
    }
