...
export default function alloy () {
    ...
    _alloy.jar = function (jar) {
        ...
        if (!arguments.length || jar === 'none') return _jar;
    ...
    _alloy.visualize = function (command, solution) {
        if (_is_java_running) {
            try {
                temp.open('alloy', function (error, info) {
                    if (error) {
                        console.log(error);
                    } else {
                        solution.writeXMLSync(info.path);
                        Java.newInstanceSync('edu.mit.csail.sdg.alloy4viz.VizGUI', false, info.path, null, null, null);
                    }
                });
            }
            catch (e) {
