...
export default class AlloyView {
    ...
    constructor(runner) {
        ...
        // Views
        this.view_settings_required = settings_required_view(this.selection);
        this.view_java_loading = java_loading_view(this.selection);
        this.view_main = main_view(this.selection);
        this.view_needs_restart = needs_restart_view(this.selection);
    ...
    destroy() {
        // Disposables and children with disposables
    ...
    showRestart(reason) {
        if (reason === 'newjar') {
            ...
            this.view_java_loading.hide();
            this.view_main.hide();
            this.view_needs_restart.show_warning(() => this.showMain());
        }
    }
    ...
    setRunner(runner) {
        ...
        if (runner) {
            ...
            this.runner.onParseCommands((result) => {
                this.view_main.setCommands(result.editor, result.commands)
            });

            // Subscribe to events from the views
            this.view_main.onRunCommand((command) => { console.log(command); });
