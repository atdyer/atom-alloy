...
export default function alloy_view () {
    ...
    function _create_alloy_view (element) {
        ...
        _view_main_subscriptions.add(
            _view_main.on_command_clicked(_on_command_clicked)
        );
    ...
    function _on_command_clicked (selection) {

        console.log(selection);
        return;

        // Color the command as running
        selection.classed('running', true);

        let d = selection.datum();

        if (_runner) {
            _runner.execute(d.file, d.command);
        }
    }
